...
@section('content')
    <div class="panel panel-default">
        <div class="panel-heading">
            ...
            <span class="panel-tit">部门管理</span>
        </div>

        <div class="panel-body navbar-form">
            <!-- 搜索栏 -->
            <form method="get" action="{{ route('admin.departments.index') }}" id="search">
                <input type="text" name="name" placeholder="部门名称" class="form-control input-sm" value="{{ request('name') }}">
...
@section('script')
    <script>
        ...
        $(function(){
            // 单个删除
            $("#department_list").on('click', ".del_department", function(){
            ...
            //更换状态
            $("#department_list").on('click', ".change-status", function(){
                let id = $(this).data("id");
                ...
                $.ajax({
                    url: "{{ route('admin.departments.status','') }}/"+id,
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        status: status == 1 ? 0 : 1
        ...
        function Alldel() {
            ...
            if(!confirm("确定要删除选中部门吗？")) return false;

            $.ajax({
                url: "{{ route('admin.departments.batch.destroy') }}",
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    ids: ids,
