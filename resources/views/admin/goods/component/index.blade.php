...
@section('content')
    <div class="panel panel-default">
        <div class="panel-heading">
            ...
            <span class="panel-tit">商品材质管理</span>
        </div>

        <div class="panel-body navbar-form">
            <!-- 搜索栏 -->
            <form method="get" action="{{ route('admin.goods.components.index') }}" id="search">
                <input type="text" name="name" placeholder="中文名称" class="form-control input-sm" value="{{ request('name') }}">
                ...
                <!-- 新增按钮 -->
                <a href="{{ route('admin.goods.components.create') }}" class="btn btn-primary btn-sm pull-right">
                    <span class="glyphicon glyphicon-plus"></span> 新增材质
                </a>
            </form>
            ...
            <table class="table table-bordered table-hover table-striped" id="log_form">
                ...
                @foreach($list as $key => $vo)
                    <tr class="text-center">
                        ...
                        <td>
                            <a href="{{ route('admin.goods.components.edit', $vo) }}" class="text-info m-r-1">
                                <span class="glyphicon glyphicon-edit"></span> 编辑
                            </a>
                            <a href="javascript:;" class="delete del_component" data-id="{{ $vo->id }}">
...
@section('script')
    <script>
        ...
        $(function(){
            // 单个删除
            $("#component_list").on('click', ".del_component", function(){
                ...
                $.ajax({
                    url: "{{ route('admin.goods.components.destroy', '') }}/" + id,
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        _method: "DELETE"
            ...
            //更换状态
            $("#component_list").on('click', ".change-status", function(){
                let id = $(this).data("id");
                ...
                $.ajax({
                    url: "{{ route('admin.goods.components.status','') }}/"+id,
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        status: status == 1 ? 0 : 1
        ...
        function Alldel() {
            ...
            if(!confirm("确定要删除选中材质吗？")) return false;

            $.ajax({
                url: "{{ route('admin.goods.components.batch.destroy') }}",
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    ids: ids,
