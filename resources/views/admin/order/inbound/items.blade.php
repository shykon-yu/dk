...
@section('content')
    <div class="panel panel-default">
        <div class="panel-heading">
            ...
            <span class="panel-tit">入库子单管理</span>
        </div>

        <div class="panel-body navbar-form">
            <!-- 搜索栏 -->
            <form method="get" action="{{ route('admin.inbounds.items') }}" id="search" class="form-inline" style="gap:8px; display:flex; flex-wrap:wrap; align-items:center;">
            ...
            <table class="table table-bordered table-hover table-striped" id="items_table">
                ...
                @foreach($items as $key => $item)
                    <tr class="text-center">
                        ...
                        <td>{{ $key + 1 }}</td>
                        <td>{{ $item->inbound->customer->name ?? '-' }}</td>
                        <td>{{ $item->inbound->supplier->name ?? '-' }}</td>
                        <td>
                        ...
                        <td>{{ $item->price }}</td>
                        <td>{{ $item->amount }}</td>
                        <td>{{ $item->inbound->inbound_code ?? '-' }}</td>
                        <td>{{ $item->orderItem->order->order_code ?? '-' }}</td>
                        <td>{{ $item->inbound->inbound_at_date ?? '-' }}</td>
                        <td>{{ $item->inbound->creator->name ?? '系统' }}</td>
                    </tr>
                @endforeach
                </tbody>
                <tfoot>
    ...
    <script>
        $(function () {
            ...
            $('#R').click(function (e) {
                ...
                window.location.href = "{{ route('admin.inbounds.items') }}";
            });
        });
    </script>
@endsection
