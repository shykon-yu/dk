...
@section('content')
    <div class="panel panel-default">
        <div class="panel-heading">
            ...
            <span class="panel-tit">支付方式管理</span>
        </div>

        <div class="panel-body navbar-form">
            <!-- 搜索栏 -->
            <form method="get" action="{{ route('admin.payments.index') }}" id="search">
                <input type="text" name="name" placeholder="支付方式名称" class="form-control input-sm" value="{{ request('name') }}">
                ...
                <a href="{{ route('admin.payments.create') }}" class="btn btn-primary btn-sm pull-right">
                    <span class="glyphicon glyphicon-plus"></span> 新增支付方式
                </a>
            </form>
            ...
            <table class="table table-bordered table-hover table-striped" id="log_form">
                ...
                @foreach($list as $key => $vo)
                    <tr class="text-center">
                        ...
                        <td>
                            <a href="{{ route('admin.payments.edit', $vo) }}" class="text-info m-r-1">
                                <span class="glyphicon glyphicon-edit"></span> 编辑
                            </a>
                            <a href="javascript:;" class="delete del_payment" data-id="{{ $vo->id }}">
...
@section('script')
    <script>
        ...
        $(function(){
            ...
            // 单个删除
            $("#payment_list").on('click', ".del_payment", function(){
            ...
            //更换状态
            $("#payment_list").on('click', ".change-status", function(){
                let id = $(this).data("id");
                ...
                $.ajax({
                    url: "{{ route('admin.payments.status','') }}/"+id,
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        status: status == 1 ? 0 : 1
        ...
        function Alldel() {
            ...
            if(!confirm("确定要删除选中支付方式吗？")) return false;

            $.ajax({
                url: "{{ route('admin.payments.batch.destroy') }}",
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    ids: ids,
